...
function bestwebsite_get_redirects( $args = array(), $hard = false ) {

	$redirects = get_transient( '_bestwebsite_redirects' );
	...
	if ( $hard || false === $redirects ) {
		...
		$default_max_redirects = apply_filters( 'bestwebsite_max_redirects', 250 );
		...
		set_transient( '_bestwebsite_redirects', $redirects );
	}
 ...
 * @return bool
 */
function bestwebsite_max_redirects_reached() {
	$default_max_redirects = apply_filters( 'bestwebsite_max_redirects', 250 );

	$redirects = bestwebsite_get_redirects();

	return ( count( $redirects ) >= $default_max_redirects );
}


function bestwebsite_get_valid_status_codes() {
	return apply_filters( 'bestwebsite_valid_status_codes', array( 301, 302, 303, 307, 403, 404 ) );
}


function bestwebsite_flush_cache() {
	delete_transient( '_bestwebsite_redirects' );
}



function bestwebsite_check_for_possible_redirect_loops() {
	$redirects = bestwebsite_get_redirects();
...
function bestwebsite_create_redirect( $redirect_from, $redirect_to, $status_code = 302, $enable_regex = false, $post_status = 'publish', $menu_order = 0 ) {
	global $wpdb;

	$sanitized_redirect_from = bestwebsite_sanitize_redirect_from( $redirect_from );
	$sanitized_redirect_to   = bestwebsite_sanitize_redirect_to( $redirect_to );
	...
	if ( ! in_array( $sanitized_status_code, bestwebsite_get_valid_status_codes(), true ) ) {
...
function bestwebsite_sanitize_redirect_to( $path ) {
	$path = trim( $path );
...
function bestwebsite_sanitize_redirect_from( $path, $allow_regex = false ) {
...
function bestwebsite_import_file( $file, $args ) {
	$handle       = $file;
	$close_handle = false;
	$doing_wp_cli = defined( 'WP_CLI' ) && WP_CLI;

	// filter arguments
	$args = apply_filters( 'bestwebsite_import_file_args', $args );
	...
	while ( ( $row = fgetcsv( $handle ) ) ) {
		...
		// sanitize
		$redirect_from = bestwebsite_sanitize_redirect_from( $rule[ $args['source'] ] );
		$redirect_to   = bestwebsite_sanitize_redirect_to( $rule[ $args['target'] ] );
		...
		// import
		$id = bestwebsite_create_redirect( $redirect_from, $redirect_to, $status_code, $regex, 'publish', $menu_order );
...
function bestwebsite_match_redirect( $path ) {
	return bestwebsite_Redirect::factory()->match_redirect( $path );
}
